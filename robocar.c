...
unsigned long delay, IN1,IN2;// input from PD0,PD1
...
int main(void)
{
	...
	while(1)
	{
		...
		if((!IN1)&&(!IN2)) // both not pressed
			GPIO_PORTB_DATA_R = 0x05;// MOVE FORWARD
		else if((IN1)&&(!IN2))// PD0 IS PRESSED left bump
			GPIO_PORTB_DATA_R = 0x06;// TURN RIGHT
		else if((!IN1)&&(IN2))//PD1 IS PRESSED right bump
			GPIO_PORTB_DATA_R = 0x09;// TURN LEFT
...
void PortB_Init(void)
{
	...
	GPIO_PORTB_PCTL_R &= (~0xFFFF);//bits for PB3 PB2 PB1 PB0
...
void PortD_Init(void)
{
	...
	delay = SYSCTL_RCGC2_R; // wait 3-5 bus cycles
	GPIO_PORTD_DIR_R |= 0x00; // PD1 PD0 output
	GPIO_PORTD_AFSEL_R &= (~0x0F); // not alternative
	GPIO_PORTD_AMSEL_R &= (~0x0F); // no analog
	GPIO_PORTD_PCTL_R &= (~0xFFFF);//bits for PD1 PD0
	GPIO_PORTD_DEN_R |= 0x0F; // enable PD1 PD0
}
